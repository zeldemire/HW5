...
FILE*
PNG::validateHeader(const char* fileName) throw(std::runtime_error) {
    ...
    fread(pngHeader, sizeof(char), 8, pngFile);
...
void
PNG::open(const char* fileName) throw(std::runtime_error) {
    ASSERT(pngInfo == NULL);
    ...
    if (pngInfo->color_type != PNG_COLOR_TYPE_RGBA) {
...
void
PNG::load(const std::string& fileName) throw(std::runtime_error) {
    open(fileName.c_str());
}
...
void
PNG::create(int width, int height) throw(std::runtime_error) {
    this->width  = width;
    this->height = height;
...
void
PNG::write(const std::string& fileName) throw(std::runtime_error) {
